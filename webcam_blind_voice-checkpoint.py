      ...
      for i,b in enumerate(boxes[0]):
        ...
        if classes[0][i] ==44:
            if scores[0][i] >= 0.5:
                ...
                speak(f"{apx_distance} units")  # Use the speak function
                speak("BOTTLE IS AT A SAFER DISTANCE") 

                if apx_distance <=0.5:
                    if mid_x > 0.3 and mid_x < 0.7:
                        cv2.putText(image_np, 'WARNING!!!', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,255), 3)
                        print("Warning -BOTTLE very close to the frame")
                        speak("Warning -BOTTLE very close to the frame")
        ...
        if classes[0][i] ==1:
            if classes[0][i] == 1:  # Person
                if scores[0][i] >= 0.5:
                    ...
                    print(apx_distance)
                    speak(f"{apx_distance} units")  # Use the speak function
                    speak("Person is AT A SAFER DISTANCE")  # Use the speak function
        
                if apx_distance <= 0.5:
                   if mid_x > 0.3 and mid_x < 0.7:
                      cv2.putText(image_np, 'WARNING!!!', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 3)
                      print("Warning - Person very close to the frame")
                      speak("Warning - Person very close to the frame")  # Use the speak function
